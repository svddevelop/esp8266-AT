...
static BOOL at_ipMux = TRUE;//FALSE;
...
static int8_t ipLastOp = -1;
/** @defgroup AT_IPCMD_Functions
  * @{
  */

int8_t ICACHE_FLASH_ATTR at_getLastOp(void){
	return ipLastOp;
}
...
/**
  ...
  * @retval None
  */
extern volatile at_sharedMem_t sharedMem;
extern volatile uint8_t svdStatus;
extern at_setCmdLsharedmem(uint8_t id, char *pPara);
#define _SM_DBLBUF	_SM_BUF_LEN * 2
    ...
    if (len > 2){
    	if (pdata[0] == 'E'){
    		...
    		if (_CHECKBIT(svdStatus, _SVD_STATUS_ECHO)){
    			...
    			uart0_tx_buffer(echoCmd, len-1);
    			uart0_sendStr("\r\n");
    		}
    		at_cmdProcess(echoCmd);
    		chk = 1;
    		return;
    		...
    		//возвращает имя станции
    		  ...
    		  while((apConfig.ssid[len] > 32)&(len < sizeof(apConfig.ssid))) len++;
			
    		  espconn_sent(arg, apConfig.ssid, len);
    		  return;
    		
    	}
    	else if (pdata[0] == 'G'){
    		
    		uint8_t len = 0; char text [2]; char textBuf [_SM_DBLBUF];
    		while (len < _SM_BUF_LEN){

    				os_sprintf(text, "%02X", sharedMem.buf[ len ]);
    				//uart0_sendStr(text);uart0_sendStr("\r\n");
    				textBuf[ len * 2] = text[0];
    				textBuf[len * 2 +1] = text[1];
    				len++;
    		}
    		//uart0_sendStr(textBuf);
    		
    		//espconn_sent(pLink[sendingID].pCon, textBuf, sizeof(textBuf));
    		espconn_sent(arg, textBuf, _SM_DBLBUF);
    		return;
    		...
    		// устанавливается сразу 8 байт. Иначе пишет всякий мусор
    		//
    		at_setCmdLsharedmem(33, pdata);    	
    		return;
    	}
    }
    if (chk == 0){
