...
#define at_backTeError   "+CTE ER: %d\r\n"
...
typedef struct
{
  ...
  at_passtr_t 	remoteIP;
  at_passtr_t 	remoteGET;
  uint16_t		remotePort;
  
}at_uartType;

#define _SM_BUF_LEN	8
typedef struct {
	
	char buf [8];
	uint8_t changedByUART : 1;
	uint8_t changedByTCP  : 1;	
	
	uint8_t unused_bit    : 6; //only for allignment
	uint8_t unusen_byte[3]	 ; //only for align
} at_sharedMem_t;

void at_init(void);
void at_cmdProcess(uint8_t *pAtRcvData);

#define _CHECKBIT(x,y)		(x & (1 << y))
#define _SVD_STATUS_ECHO	1
